...
namespace ly {
	HexagonStage::HexagonStage(World* world)
		...
		mRigthSpawnLoc{ world->GetWindowSize().x / 2.f + mSideSpawnOffset, -100.f - mSideSpawnOffset },
	...
	void HexagonStage::SpawnHexagon()
	{
		...
		newHexagon = GetWorld()->SpawnActor<Hexagon>();
		newHexagon.lock()->SetActorLocation(mRigthSpawnLoc);

		if (++mCurrentSpawnCount == mSpawnGroupAmot) {
			FinishStage();
		}
	}
