...
namespace ly {
	...
	{
		mSpawnTimer = TimerManager::Get().SetTimer(GetWeakRef(), &ChaosStage::SpawnVanguard, mMinSpawnInterval);
	...
	void ChaosStage::SpawnUFO()
	{
		...
		sf::Vector2f spawnLoc = GetRandomSpwanLocationSide();
	...
	{
		auto windowSize = GetWorld()->GetWindowSize();
		float spwanX = RandomRange(100.f, windowSize.x - 100.f);
		float spwanY = -100.f;
		return { spwanX, spwanY };
	}
	sf::Vector2f ChaosStage::GetRandomSpwanLocationSide() const
	{
		auto windowSize = GetWorld()->GetWindowSize();
