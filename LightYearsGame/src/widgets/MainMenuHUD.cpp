...
namespace ly {
	...
	void MainMenuHUD::Init(const sf::RenderWindow& windowRef)
	{
		...
		mTitleText.SetWidgetLocation({ windowSize.x / 2.f - mTitleText.GetBound().width/2, 100.f});
		mStartButton.SetWidgetLocation({ windowSize.x / 2.f - mStartButton.GetBound().width / 2, windowSize.y / 2.f });
		mQuitButton.SetWidgetLocation(mStartButton.GetWidgetLocation() + sf::Vector2f{ 0.f, 50.f });
		mStartButton.onButtonClicked.BindAction(GetWeakRef(), &MainMenuHUD::StartButtonCliked);
		mQuitButton.onButtonClicked.BindAction(GetWeakRef(), &MainMenuHUD::QuitButtonCliked);

	}
	void MainMenuHUD::StartButtonCliked()
	{
		onStartButtonCliked.Broadcast();
	}
	void MainMenuHUD::QuitButtonCliked()
	{
